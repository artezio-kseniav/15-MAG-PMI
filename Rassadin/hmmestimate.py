    ...
    def train(self, observations, criterion, graphics=False):
        ...
        while not done:
            ...
            for t in range(1, nSamples):
                alpha[:, t] = np.dot(
                    alpha[:, t - 1].T, self.A).T * self.B[:, observations[t]]
            ...
            prod = (alpha[:, nSamples - 1] *
                    beta[:, nSamples - 1]).reshape((-1, 1))
...
if __name__ == '__main__':
    ...
    plt.cla()
    plt.plot(
        np.vstack((s * 0.9 + 0.05, hmmguess.gamma[1, :])).T, '-o', alpha=0.7)
